# Agent Rules for Apache Pekko Persistence JDBC
...
- Database schema changes must include migration scripts and backward compatibility considerations.
- Changes must be tested against at least one supported database (PostgreSQL, MySQL, SQL Server, Oracle, H2).
...
- Integration tests require a running relational database. If unavailable, record in `Tests`.
