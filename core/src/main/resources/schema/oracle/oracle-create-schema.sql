...
CREATE
OR REPLACE TRIGGER DURABLE_STATE__GLOBAL_OFFSET_SEQ_TRG before insert on DURABLE_STATE REFERENCING NEW AS NEW FOR EACH ROW WHEN (new.GLOBAL_OFFSET is null)
BEGIN
    select DURABLE_STATE__GLOBAL_OFFSET_SEQ.nextval
    into :new.GLOBAL_OFFSET
    from sys.dual;
END
/
